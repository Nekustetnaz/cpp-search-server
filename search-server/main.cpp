...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentFilter filter) const {
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus request_status) const {
        auto matched_documents = FindTopDocuments(
            raw_query,
            [request_status](int document_id, DocumentStatus document_status, int rating) { 
                return request_status == document_status; 
            });
        return matched_documents;
    }
...
private:
    ...
    template <typename DocumentFilter>
    vector<Document> FindAllDocuments(const Query& query, DocumentFilter filter) const {
...
int main() {
    ...
    search_server.AddDocument(
        0, "белый кот и модный ошейник"s,        DocumentStatus::ACTUAL, {8, -3}
    );
    search_server.AddDocument(
